...
class RAGEngine:
    ...
    def build_index(self, chunks: List[Dict[str, Any]]):
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            ids = ["chunk_" + str(i + j) for j in range(len(batch))]
    ...
    def query(self, question: str, history: List[Dict] = None) -> Dict[str, Any]:
        ...
        if results["documents"] and results["documents"][0]:
            for j, (doc, meta) in enumerate(zip(results["documents"][0], results["metadatas"][0])):
                ...
                sources.append({"source": src, "content": doc})
                context_parts.append("[Source " + str(j+1) + ": " + src + "]\n" + doc)
        context = "\n\n---\n\n".join(context_parts)
        system_msg = (
            "You are a helpful assistant that answers questions based on provided documents. "
            "Use the context below to answer. Always cite which source document you used. "
